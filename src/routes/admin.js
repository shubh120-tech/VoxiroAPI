...
router.get("/support", async (req, res) => {
  try {
    ...
    const offset = (page - 1) * limit;
    let sql = `SELECT st.*, b.name AS business_name FROM support_tickets st
               JOIN businesses b ON b.id = st.business_id WHERE 1=1`;
    const params = [];
...
router.patch("/support/:id/resolve", async (req, res) => {
  try {
    await query(`UPDATE support_tickets SET status='resolved', resolved_by=$1, resolved_at=NOW(), notes=$2 WHERE id=$3`,
