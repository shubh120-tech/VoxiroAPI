  ...
  try {
    ...
    await query(`
      INSERT INTO subscriptions
        (business_id, plan_id, billing_cycle_start, billing_cycle_end,
      ...
      ON CONFLICT (business_id) DO UPDATE SET
        ...
        is_active           = TRUE,
        updated_at          = NOW()
    `, [bId, plan.id, now, cycleEnd, trialEnd]);
...
router.post("/billing/webhook", express.raw({ type: "application/json" }), async (req, res) => {
  try {
    ...
    if (event.event === "payment.captured" && payload) {
      ...
      await query(`
        UPDATE payment_history
        SET razorpay_payment_id = $1, paid_at = NOW(), is_active = TRUE, updated_at = NOW()
