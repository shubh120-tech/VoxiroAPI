  ...
  try {
    ...
    let sql = `
      SELECT
        ...
        (SELECT COUNT(*) FROM support_ticket_comments c WHERE c.ticket_id = t.id AND c.is_internal = FALSE)::int AS comment_count,
    ...
    const count = await query(
      `SELECT COUNT(*) FROM support_tickets WHERE business_id = $1${status ? ` AND status = '${status}'` : ""}`,
      [bId(req)]
    );
...
router.get("/tickets/:id", async (req, res) => {
  try {
    ...
    const [comments, attachments, statusLog] = await Promise.all([
      query(`SELECT * FROM support_ticket_comments WHERE ticket_id = $1 AND is_internal = FALSE ORDER BY created_at ASC`, [req.params.id]),
...
router.post("/tickets", async (req, res) => {
  try {
    ...
    const { rows } = await query(`
      INSERT INTO support_tickets (business_id, created_by, subject, description, category, priority, status)
...
router.post("/tickets/:id/comments", async (req, res) => {
  try {
    ...
    const { rows: t } = await query("SELECT id FROM support_tickets WHERE id=$1 AND business_id=$2", [req.params.id, bId(req)]);
...
router.post("/tickets/:id/close", async (req, res) => {
  try {
    const { rows: t } = await query(
      "SELECT id, status FROM support_tickets WHERE id=$1 AND business_id=$2",
      [req.params.id, bId(req)]
    );
    if (!t.length) return res.status(404).json({ message: "Ticket not found" });
